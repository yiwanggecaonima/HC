...
class ProxyMiddleware():
    ...
    def get_proxy(self):
        try:
            headers = {'User_Agent': random.choice(agents)}
            Request = urllib.request.Request(self.PROXY_URL, headers=headers)
        ...
        if request.meta.get('retry_times') == 4:
            ...
            try:
                ...
                if self.proxy:
                    ...
                    self.logger.debug("this is ip:" + self.proxy)
                    # 如果此处是私密ip,需要做个判断是否带有用户名密码
                    # return request
    ...
    # def process_exception(self,request, exception, spider):
    #     self.logger.debug('IP被干掉,更换IP ...... ')
    #     proxy = self.get_proxy()
    #     request.meta['Proxy'] = 'https://' + proxy
    #     return request


class HcItemSpiderMiddleware(object):
...
class HcItemDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
